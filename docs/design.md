...
**实测结论（2026-09 于真实直播间 22301377 抓包）**：

| 字段 | 实测结果 |
|---|---|
| `info[2][0]`（uid） | **恒为 0** —— B 站已对未登录观众隐藏 |
| `info[2][1]`（昵称） | **被打码**，形如 `赛***`、`x***` |
| `info[0][15].extra.user_hash` | **4/4 都有**，且同一用户稳定 |

若按直觉以 `uid` 为先着色，**全场观众会塌成同一个颜色**。
