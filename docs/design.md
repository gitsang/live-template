...
| 网页登录口令 | `loginToken` | `LOGIN_TOKEN` | — | 空（关闭网页登录） | 访问口令；留空即关闭网页登录入口；⚠️ 与 HTTPS 无关，见上文 |
...
管理页是独立的 `/admin`，需 `LOGIN_TOKEN` 进入（口令 → 签名会话 Cookie → 页面）。

##### 为什么把它从 OBS 画布的 HUD 里搬出来

最初登录按钮放在 HUD 上。当时的安全性依赖两个**间接**前提：
HUD 默认关闭、且 OBS 浏览器源不派发鼠标事件（所以即使开着也不会出现在画面里）。

这是「依赖外部行为」的保证，不牢靠 —— 一旦 OBS 行为变化、或有人把
`?hud=1` 的地址填进浏览器源，二维码就可能出现在直播画面里。

独立页面是**结构性**保证：画布上根本没有这个东西。
管理动作（登入 B 站）本来也不该与画面渲染耦合。

HUD 上只留一个指向 `/admin` 的普通链接（`target="_blank"`）。

##### 为什么不把口令存在浏览器里

早期实现让前端把 `LOGIN_TOKEN` 存进 `sessionStorage`，每次请求带
`x-login-token` 头。问题在于：**口令是长期凭据**，一旦被读到
（XSS、误贴、浏览器扩展）就等于永久泄漏；而且前端 JS 每持有它一秒，
暴露面就大一分。

现在改成：口令只在登入时提交一次，服务端校验通过后下发
**HttpOnly + HMAC 签名** 的会话 Cookie。之后浏览器自动携带，JS 读不到也不需要读。

用签名而非服务端 session 表，是因为签名无状态：重启后仍有效、无需清理过期条目。
密钥由 `LOGIN_TOKEN` 派生，因此**轮换口令即等于吊销所有会话** ——
这正是需要的语义。代价是不能单独吊销某一个会话，单运营者场景下无所谓。

##### `Secure` 属性必须按实际协议决定（实测踩坑）

SvelteKit 的 `cookies.set` 默认 `secure: true`，除了 `localhost` 上的 http。
而本项目 **compose 默认把端口发布到 `0.0.0.0`**，即从
`http://192.168.x.x:8080` 访问 —— 此时若带上 `Secure`，浏览器会**直接丢弃**
这个 Cookie，表现为「登入提示成功，但页面依旧未授权」，极难排查。

所以显式按「实际协议（含 `X-Forwarded-Proto`）」决定。

删除 Cookie 时同样要带上一致的属性：实测不传 `secure` 时默认成 `true`，
于是 http 下浏览器**忽略删除指令，登出后会话依旧有效**。
（当时响应是 `lt_admin=; Max-Age=0; ...; Secure; SameSite=Lax`，
而设置时是 `SameSite=Strict` —— 属性不一致。）

##### 授权边界

- 未授权时 `/admin` 只渲染登录表单，**连「B 站是否已登录」都不下发**。
- 所有 `/api/login/*` 都要求有效管理会话；漏一个就等于全没防，
  因此校验统一走 `checkAdminSession()`。
- 管理会话与核心渲染路径解耦：OBS 页面（`/`、`/only/*`）不需要任何授权。



##### 先说清楚：口令防不住二维码钓鱼

最初的实现注释把加口令的理由写成「防 QRLJacking 钓鱼」，**这是错的**，已更正。

B 站的生成接口是**公开的**，任何人三行代码就能自己造一个码，根本不需要经过本服务：

```
GET https://passport.bilibili.com/x/passport-login/web/qrcode/generate
→ 无需任何认证，直接返回 { url, qrcode_key }   （已实测：code=0，HTTP 200）
```

所以攻击者可以绕开我们造码、并诱导操作者去扫 —— 这是二维码登录**固有的**属性，
服务端无法阻止。把它写成口令的理由是站不住的。
...
```
GET /x/passport-login/web/qrcode/poll?qrcode_key=<key>
→ 无 Cookie、无 Referer 校验，返回 86101（未扫码）
```

也就是说，**持有 `qrcode_key` 的人就能在扫码成功后领走凭据（Set-Cookie）**。
生成挑战与轮询之间没有任何绑定。

**实测事实二：二维码图就是 key。**

把本服务下发的 SVG 解码回来，可以还原出 `qrcode_key`，并可拿去独立轮询（已实测成功）。
所以「只暴露二维码、不暴露 key」这种想法提供不了任何保护 —— **二维码只是 key 的图形编码**，
两者是同一个东西。

两条合起来，得到真实的攻击路径（**同一网络内**即可实施）：

1. 操作者打开登录页，服务端生成挑战并显示二维码
2. 同网段的攻击者调 `GET /api/login/status?svg=1`，取到**操作者正在扫的那张图**
3. 攻击者解码图片还原出 key，自己去 B 站轮询这个 key
4. 操作者扫码确认，凭据（Set-Cookie）也被攻击者的轮询收走

注意 compose **默认把端口发布到 `0.0.0.0`**，因此这个接口默认是局域网可达的。
口令就是为这个场景准备的：保护一个「凭据等价物」的分发接口。

顺带因此收掉了一个设计缺陷：轮询接口原先支持 `?svg=1` 重发二维码，但它**没有任何调用方**
（前端只在开起时拿一次图），却让凭据等价物可以被反复取回。已移除。
需要重新展示时应当重新开起挑战，而不是重发旧码。

##### 口令的实现约定

- **默认关闭**：不配置 `LOGIN_TOKEN` 就完全没有网页登录入口。
  隐式开启的认证入口是安全上的坏味道。
- 口令只在启动日志里打印一次，供人工抄进网页。
- 比较用 `crypto.timingSafeEqual`（恒定时间），避免通过响应耗时逐字节猜口令。
- 前后空白容错（口令要人工抄写）。
- 前端把口令存在 `sessionStorage`（关掉标签页即失效），**不用 localStorage**。
- 未授权请求一律 401，且**只记录「被拒绝」这一事实，不记口令内容** ——
  否则日志采集系统里就留下了可用于爆破的样本。

仍然保留终端扫码（`npm run login`）作为无口令的替代路径：
它天然安全，因为挑战与轮询都在同一个本地进程里。

##### 其它实现要点

- 接口返回**渲染好的 SVG** 而不是二维码里的 URL。要诚实地说：这**不是**安全边界 ——
  图就是 key，两者等价（见上文实测）。它只是个实现选择：省掉前端一个二维码依赖，
  并让渲染完全由服务端控制。真正的保护是那个访问口令。
- **只保留一个活动挑战**：同时存在多个既无使用场景，又会让「屏幕上这个码是哪个」
  变得含糊。
- **轮询节流放在服务端**（900ms）：前端只要有个 `setInterval` 就能把 B 站接口
  打到限流，而前端代码是可以被绕过的。
- **成功路径立即置 `settled`**：否则并发轮询会让 `onSuccess` 被重复调用，
  重复写凭据文件、重复重连采集。
- **`timeout` 与 `expired` 是两个状态**：混淆会输出
  「二维码已过期（状态码 86101）」这种自相矛盾的文案（86101 是未扫描）。
- 二维码静默区强制不低于 4 个模块（规范下限），传 0 也不会画出无法识别的码。
- SVG 里的颜色值会被 XML 转义 —— 颜色来自配置，直接拼接就是注入点。

##### 登录后热重载

登录态是在认证包（op=6/7）里一次性发出的，已建立的连接**无法中途改变身份**，
因此必须断开重连。

`#epoch` 代次计数器在这里是必需的，而且**已实测证明**：

```
去掉 epoch 守护：reload 后新增 2 次连接（5s 后冒出一条陈旧的重连）
保留 epoch 守护：reload 后恰好 1 次
```

原因：`run()` 的 `.finally()` 是异步执行的，它决定要不要退避重连。
主动重连会把 `#stopped` 重新置为 `false`，旧回调看到后就会再安排一次重连，
于是两条连接并存、弹幕重复。代次对不上就直接放弃。

这个守护同时修掉了一个**早已存在的竞态**：空闲断开（`stop()`）后若立刻
`acquire()`，旧连接的 `.finally()` 可能还没执行，同样会重复连接。
