...
非目标（本期明确不做）：礼物渲染、醒目留言（SC）、上舰、进房欢迎、头像、用户名着色、
UL 等级、粉丝牌、屏蔽词、限流、公告框、按键映射、按键历史序列。
...
| 区域 | x | y | 宽 | 高 |
|---|---|---|---|---|
| **视频框 body** | 31 | 61 | **1360** | **765** |
| 公告框 body | 31 | 880 | 1360 | 169 |
| **聊天框 body** | 1415 | 61 | **474** | **630** |
| **手柄框 body** | 1415 | 745 | **474** | **304** |
...
`474×630`，字体 16px / 行高 1.6（单行 25.6px）→ **可见约 23 行**。
...
| 参考线 | `guides` | `GUIDES` | `?guide=1` | `false` | 尺寸标注与四角括号 |
...
| `/only/chat` | 独立聊天框 | **精确 474×630**，无画布无缩放，OBS 里直接填宽高 |
| `/only/pad` | 独立手柄框 | **精确 474×304** |
| `/only/video` | 独立视频框 | **精确 1360×765**，纯透明洞 + 标题栏 |
| `/only/notice` | 独立公告框 | **精确 1360×169**（本期空） |
...
```dockerfile
# 构建阶段：装依赖 + svelte-kit build
FROM node:22-alpine AS build
# 运行阶段：只拷 build/ + node_modules(prod) + config.json
FROM node:22-alpine
EXPOSE 8080
CMD ["node", "server.mjs"]
```
...
4. 需单独控制手柄框/聊天框时，另加 `/only/pad`（474×304）与 `/only/chat`（474×630）。
...
- [ ] 聊天框：超 23 行才滚动、自动吸底、长文换行、淡入动画平滑、DOM 不超 300 条；
