...
- [ ] 同一 uid 的用户名颜色在不同页面刷新后保持一致；相邻 uid 颜色分散；
...
- **用户名着色**用 `uid` 稳定散列到 8 色亮色板（`nameColor()`），
  而**不用** B 站下发的颜色：那套颜色面向浅色主题，深底上常看不清。
  乘散列（`imul(uid, 2654435761)`）而不是取模，避免相邻 uid 落到相邻颜色。
  uid 缺失时退回昵称散列，保证异常数据也稳定。
