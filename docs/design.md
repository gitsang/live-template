...
#### 网页扫码登录，以及为什么 HTTPS 不足以防护

网页端入口在 HUD 里（`/?hud=1` → 「登录」），二维码由**服务端**渲染成 SVG 下发。

##### 攻击面：不是中间人，是二维码钓鱼

常见误解是「上了 HTTPS 就没有这个问题了」。实际上这里的攻击与传输无关：
...
也就是说，**谁持有 `qrcode_key`，谁就能在扫码成功后收走凭据（Set-Cookie）**。
生成挑战和轮询之间没有任何绑定关系。

由此产生的攻击叫 **QRLJacking / 二维码钓鱼**：

1. 攻击者访问他自己的（或你的）登录接口，拿到一个挑战
2. 他把那个二维码展示给操作者（钓鱼页、聊天窗口、替换界面截图）
3. 操作者用 B 站 App 一扫，**凭据就被攻击者那边的轮询收走了**
4. 攻击者拿到了操作者的 B 站登录态

注意这条链路里 HTTPS 全程有效、证书完全合法 —— 攻击者走的是你自己的
合法接口，加密解决不了「你不该把码给别人扫」这件事。

##### 防护：访问口令

因此登录接口必须有访问控制，本项目用 `LOGIN_TOKEN`：
...
- 未授权的请求一律 401，且**只记录「被拒绝」这一事实，不记口令内容** ——
...
- **`qrcode_key` 绝不离开服务端**：接口返回的是渲染好的 SVG，不是二维码里的 URL。
  否则前端拿到 URL 就等于拿到了 key，任何能读到该响应的人都能抢走凭据。
