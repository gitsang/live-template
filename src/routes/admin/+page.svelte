<script lang="ts">
	 ...
	 * 管理页 /admin
	 *
	 * 为什么单独开一个页面、而不是继续放在 OBS 画布的 HUD 上：
	 *
	 * 1. **二维码绝不能出现在直播画面里**。HUD 虽然默认关闭、且只在鼠标移动时
	 *    出现（OBS 浏览器源不派发鼠标事件），但那是「依赖 OBS 行为」的间接保证。
	 *    把它移到独立页面才是结构性保证 —— 画布上根本没有这个东西。
	 * 2. 管理动作（登入 B 站、改配置）本来就不该跟画面渲染耦合。
	 * 3. 口令只在这里出现一次，换成 HttpOnly 会话 Cookie，不再进 sessionStorage。
	 *
	 * 本页不参与画布缩放：全局样式里 body 是 `overflow: hidden`，
	 * 这里自己开一个可滚动容器。
	 */
