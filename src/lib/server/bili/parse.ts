/**
 * B 站弹幕服务消息 → 本项目事件的纯解析函数。
 *
 * 抽出来的原因：
 * 1. 可单测 —— 解析写在私有方法里就只能靠连真实直播间碰运气，
 *    而 SC（醒目留言）本身很稀有，礼物也要等；用固定报文做测试才可靠。
 * 2. B 站同一份数据在不同指令里字段不同名（粉丝牌有数组/对象两种形态，
 *    等级在 gift 里叫 wealth_level、在 SC 里叫 user_level），
 *    集中在一处便于逐项兼容并留下记录。
 *
 * 字段来源：B 站公开的直播间消息格式 + scripts/danmaku.py（可用的参考实现）。
 * 取值的策略是「多路径兜底」：任一字段缺失都不应让整条事件丢掉。
 */
